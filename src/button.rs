...
use crate::error::WidgetCreationError;
...
pub struct ButtonDescriptor<T> {
    ...
    pub render_interface: RenderInterface,
...
pub struct Button<T> {
    /// Graphical sprite for the button.
    sprite: Sprite,
...
impl<T> Button<T> {
    ...
    /// Create a new button.
    pub fn new(descriptor: ButtonDescriptor<T>) -> Result<Self, WidgetCreationError> {
        let sprite = Sprite::new(
            descriptor.render_interface,
            renderer::SHADER_ID_UI,
            descriptor.position,
            descriptor.size,
        )
        .map_err(|_| WidgetCreationError::SpriteCreation)?;
